...
public class Serial_02 extends AppCompatActivity {
    ...
    public void send_msg(final View view) {
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                if (view.getId() == R.id.btn_led_r)
                    pw.println("r");
                else if (view.getId() == R.id.btn_led_g)
                    pw.println("g");
                else if (view.getId() == R.id.btn_led_b)
                    pw.println("b");
            }
        });
        thread.start();
    ...
    class AsyncTaskExam extends AsyncTask<Integer, String, String> {
        ...
        protected String doInBackground(Integer... integers) {
            try {
                ...
                socket = new Socket("70.12.225.188", 12345);
                if (socket != null) {
                    ioWork();
                }
